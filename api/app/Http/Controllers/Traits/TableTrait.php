...
trait TableTrait
{
    ...
    protected function getQuery($params, $builder, $fields = ['*'], $flagOrderBy = false)
    {
        ...
        $this->_setOrderByDefault($query_builder);

        return $query_builder;
    }

    protected function _setOrderByDefault(&$query_builder)
    {
        $query_builder->orderBy('updated_at', 'DESC');
    }
            ...
            foreach ($queries as $field => $query) {
                if (is_array($query)) {
                    if (!empty($query['table'])) {
                        $field = $query['table'] . '.' . $field;
                    }

                    if (is_array(@$query['value'])) {
                        if ($this->_isFieldDate($field, $query)) {
                            $date_format = @$query['type'] == 'datetime' ? 'd/m/Y H:i' : 'd/m/Y';
                            if (trim(@$query['value'][0]) != '') {
                                $start = Carbon::createFromFormat($date_format, $query['value'][0])->startOfDay();
                                $q->where($field, '>=', $start);
                            }
                            if (trim(@$query['value'][1]) != '') {
                                $end = Carbon::createFromFormat($date_format, $query['value'][1])->endOfDay();
                                $q->where($field, '<', $end);
                            }
                        } elseif (strtolower($query['compare']) == 'in') {
                        ...
                        if ($this->_isFieldDate($field, $query)) {
                            ...
                            if ($query['compare'] == '>=' || $query['compare'] == '>') {
                                $value = Carbon::createFromFormat($date_format, $query['value'])->startOfDay();
                            } else {
                                $value = Carbon::createFromFormat($date_format, $query['value'])->endOfDay();
                            }
    ...
    protected function _isFieldDate($field, $query) {
        if (@$query['type'] == 'datetime' || @$query['type'] == 'date') {
            return true;
        }
